    ...
    private fun initUi() {
        ...
        btnNewHelperModCard.setOnClickListener {
            ...
            activity?.let {
                ...
                intent.putExtra("CARD_DATA", clickedCardData)
                it.startActivity(intent)
            }
        }
    ...
    SingletoneToken
        ...
        cardDataRepository.deleteCard(
            ...
            override fun onResponse(call: Call<ResCards>, response: Response<ResCards>) {
                "response code : ${response.code()}, message: ${response.message()}".logDebug(this@NewHelperFragment)
                if(response.isSuccessful){
                    response.body()
                        ?.let{
                            "status : ${it.status}, success : ${it.success}, message : ${it.message}".logDebug(this@NewHelperFragment)
                            onResume()
                        }
                }
                else{
